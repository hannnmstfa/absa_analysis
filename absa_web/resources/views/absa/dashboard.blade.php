  ...
  <style>
    ...
    * {
    ...
    .logoBadge {
      ...
      border: 1px solid rgba(214, 181, 74, .30);
      box-shadow: var(--glow);
      overflow: hidden;
    }
...
<body>
  ...
  <div class="wrap">
    ...
    <section class="hero">
      ...
      <div class="heroTop">
        <div class="logoBadge">
          {{-- TARUH logo di public/logo.jpg --}}
          {{-- <img src="{{ asset('logo.jpg') }}" alt="Logo"> --}}
          <div style="color:var(--gold);font-weight:980">L</div>
        </div>
        <div class="brand">
      ...
      <form method="POST" action="{{ route('absa.analyze') }}" class="heroForm" id="analyzeForm">
        ...
        <div class="loadingEstimateTools">
          <button type="button" class="loadingEstimateReset" id="resetLoadingEstimateBtn">Reset estimasi
            loading</button>
          @if(!empty($result))
            <a href="{{ route('absa.index') }}" class="loadingEstimateReset" id="resetDashboardBtn">Reset dashboard (hapus
              tampilan hasil)</a>
          @endif
      ...
      <div class="viewToggle">
        ...
        <button type="button" id="resetCompactModeBtn" class="toggleBtn secondary">Auto</button>
      </div>
    </section>
      ...
      <div class="panel" style="grid-column:span 12" id="trendScopedArea">
        ...
        <table class="trendTable">
          ...
          <tbody id="trendPeriodeBody">
            ...
            @empty
              <tr>
                <td colspan="4" style="color:rgba(255,255,255,.65)">Belum tersedia data tren periode pada segmen ini.</td>
              </tr>
            @endforelse
          </tbody>
        </table>
      ...
      @if(!empty($segmentasi))
        <div class="panel" id="nonUserOnlySection" style="grid-column:span 12">
          ...
          <div class="mini" style="margin-bottom:10px;line-height:1.5">
            ...
            {{ $segMode === 'sudah_pakai' ? 'Hanya responden yang sudah menggunakan' : 'Seluruh data (mode fallback)' }}
            @if(!empty($segKolom))
          ...
          <table class="segmentSummaryTable" style="margin-bottom:10px;">
            <thead>
              <tr>
          ...
          @if(!empty($segNonUserInsights))
            <div class="mini" style="margin-top:12px;margin-bottom:6px"><b>Ringkasan Insight Calon Pembeli</b></div>
      ...
      <div class="panel absa">
        ...
        <div class="absaGrid">
          <div>
            ...
            <div class="legend">
              <span class="pill"><span class="sw g"></span> True Positif</span>
              <span class="pill"><span class="sw y"></span> False Positif</span>
              <span class="pill"><span class="sw r"></span> False Negatif</span>
              <span class="pill"><span class="sw n"></span> True Negatif</span>
            </div>
          </div>
        ...
        @if(!empty($variants))
          <div id="variantScopedArea">
            ...
            @if(!empty($variantRankings))
              ...
              <table class="variantRankTable" style="margin-bottom:8px;">
                <thead>
                  <tr>
                    <th style="width:12%">#</th>
                    <th style="width:28%">Varian</th>
  ...
  <script>
    (function () {
      ...
      function renderRekomendasi(items) {
        ...
        segmentRekomList.innerHTML = items.slice(0, 3).map(row => `
                <div class="rekomItem">
                  <div class="ricon">★</div>
                  <div>
                    <p class="ttl">${toTitleCase(row.aspek)}</p>
                    <p class="txt">${normalizeSentence(row.text)}</p>
                  </div>
                </div>
              `).join('');
      }
      ...
      function renderDrilldown(segmentKey, aspekKey) {
        ...
        if (posList) {
          posList.innerHTML = posItems.map(x => `<li>${x}</li>`).join('');
        }
        if (negList) {
          negList.innerHTML = negItems.map(x => `<li>${x}</li>`).join('');
        }
      }
  ...
  (function () {
    ...
    const loadingSteps = [
      ...
      },
      {
        ...
        sub: 'Model sedang menghitung insight per aspek dan segmen.'
      },
      {
